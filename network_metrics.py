...
from mininet.node import OVSKernelSwitch  # Import Open vSwitch switch class
import pyshark

class SimpleTopo(Topo):
    def build(self):
        """Build the simple topology with two hosts and a switch."""
        # Create two hosts
        h1 = self.addHost('h1')
        h2 = self.addHost('h2')
...
def analyze_traffic_and_save_metrics():
    ...
    # Reset captures after counting packets (to avoid using already iterated packets)
    h1_capture.reset()
    h2_capture.reset()
    
    h1_byte_count = sum([pkt.length for pkt in h1_capture])
    h2_byte_count = sum([pkt.length for pkt in h2_capture])
...
if __name__ == '__main__':
    # Create the network with the Open vSwitch (ovsk) controller
    topo = SimpleTopo()
    
    # Use Mininet with the OVSKernelSwitch class (this specifies Open vSwitch switch)
    ...
    # Run a simple test (ping or iperf)
